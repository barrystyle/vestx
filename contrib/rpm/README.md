...
The RPM spec file provided here is for VESTX-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://vestx.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[VESTX git repository](https://github.com/vestx/vestx) but are not part of
...
Currently only `Source10` is used, for the example `vestx.conf` file.
...
in the vestx git repository. At present only an SVG version of the VESTX
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the vestx
...
needed when 0.12.1 is released, a proper fix is already in the VESTX git
master branch.
...
idea to build VESTX against a static version of that library compiled
...
The `vestx.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `vestx-server` package. At this point, most RPM based
...
The `vestx-server` package should have SELinux support. How to properly do
...
VESTX-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/vestx/). He has been
packaging VESTX for Fedora at least since 2012.
...
4. I renamed the `vestx` package that contains the Qt GUI to `vestx-core` as
that appears to be how the general population refers to it, in contrast to
`vestx-xt` or `vestx-classic`. I wanted to make sure the general population

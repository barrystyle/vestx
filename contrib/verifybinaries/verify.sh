...
###   vestx.org and vestx.org and compares them.
...
WORKINGDIR="/tmp/vestx_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://vestx.org"
HOST2="https://vestx.org"
BASEDIR="/bin/"
VERSIONPREFIX="vestx-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'vestx-',
...
if [ $? -ne 0 ]; then
   echo "vestx.org failed to provide signature file, but vestx.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "vestx.org and vestx.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the VESTX binary release signing key installed?"
   fi

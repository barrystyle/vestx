...
bool AddMPoSScript(std::vector<CScript> &mposScriptList, int nHeight, const Consensus::Params& consensusParams)
{
    ...
    if(!pblockindex)
    {
        LogPrintf("Block index not found\n");
        return false;
    }
    ...
    if(pblockindex->IsProofOfStake())
    {
        ...
        {
            LogPrintf("Fail to solve script for mpos reward recipient\n");
...
bool CreateMPoSOutputs(CMutableTransaction& txNew, int64_t nRewardPiece, int nHeight, const Consensus::Params& consensusParams)
{
    ...
    {
        LogPrintf("Fail to get the list of recipients\n");
        return false;
    }
...
std::map<CTxDestination, std::vector<COutput>> CWallet::ListCoins() const
{
    ...
    // https://github.com/vestx/vestx/pull/10340, but that change has been
    // postponed until after https://github.com/vestx/vestx/pull/10244 to
    ...
    for (auto& outpoint : setWalletUTXO) {
        ...
        for (unsigned int i = 0; i < wtx.tx->vout.size(); i++) {
            ...
            if(fAnonymizable) {
                ...
                for(int i=0; i<Params().CollateralLevels(); i++) {
                   if(fMasterNode && wtx.tx->vout[i].nValue == (Params().ValidCollateralAmounts()[i] * COIN)) {
                      continue;
                   }
                }
                         ...
                         int& nChangePosInOut, std::string& strFailReason, const CCoinControl& coin_control, bool sign, CAmount nGasFee, bool hasSender)
{
    ...
    int nBytes;
    {
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            // change transaction isn't always pay-to-vestx-address
            CScript scriptChange;
...
bool CWallet::GetPubeKey(const CKeyID &address, CPubKey& vchPubKeyOut) const
{
    LOCK(cs_wallet);
 ...
 * https://vestxtalk.org/?topic=54527, or
 * https://github.com/vestx/vestx/pull/1393.
 */

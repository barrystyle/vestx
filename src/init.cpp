 ...
 * between vestxd, and vestx-qt and non-server tools.
*/
...
/** Sanity checks
 *  Ensure that VESTX is running in a usable environment with all
...
{
    // Make sure only a single VESTX process is using the data directory.
...
bool AppInitMain()
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if vestx is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if vestx is started while in a temporary directory.\n",
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
                ...
                fs::path qtumStateDir = GetDataDir() / "stateVestx";
                bool fStatus = fs::exists(qtumStateDir);
                const std::string dirQtum(qtumStateDir.string());
                const dev::h256 hashDB(dev::sha3(dev::rlp("")));
                dev::eth::BaseState existsQtumstate = fStatus ? dev::eth::BaseState::PreExisting : dev::eth::BaseState::Empty;
    ...
    // ********************************************************* Step 11c: update block tip in VESTX modules
    ...
    // ********************************************************* Step 11d: start thread for vestx extensions

    threadGroup.create_thread(boost::bind(net_processing_vestx::ThreadProcessExtensions, g_connman.get()));
